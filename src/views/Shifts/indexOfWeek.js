...
class ShiftScreen extends Component {
  ...
  onWeekChanged = frist => {
    console.log('propssssssssssssssss', frist);

    this.getWeekShift({
      start: moment(frist).format('MM/DD/YYYY'),
      end: moment(frist)
        .add(6, 'days')
        .format('MM/DD/YYYY'),
    });
  };
  getWeekShift = async date => {
    console.log('TCL: ShiftScreen -> datesssssssssssssss', date);
    this.setState({allData: [], showLoading: true});
    const {TOKEN, REQ_ID} = this.props.currentUser;
    let startDate = moment(date.start).format('MM/DD/YYYY');
    let endDate = moment(date.end).format('MM/DD/YYYY');
    let values = {
      REQ_TOKEN: TOKEN,
      REQ_ID,
      REQ_NAME: 'IC_GET_SHIFTS',
      REQ_PARAMS: {START_DATE: startDate, END_DATE: endDate},
    };
    ...
    this.setState({
      ...
      datesBlacklist: [...this.state.datesBlacklist, date],
    });
  };
